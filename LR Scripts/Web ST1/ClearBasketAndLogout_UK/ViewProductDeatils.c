ViewProductDeatils()
{
	...
	//web_reg_find("Text=Temporarily out of stock", "SaveCount=outofstock_count", LAST);
	//web_reg_find("Text=Extended Range", "SaveCount=extendedrange_count", LAST);
	//web_reg_find("Text=Discontinued product", "SaveCount=discontinued_count", LAST);
	...
	lr_end_transaction("UK-06-ViewProductDetailsPage",LR_AUTO);
